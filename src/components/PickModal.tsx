...
export function PickModal({ match, onClose }: PickModalProps) {
  ...
  const handleLock = async () => {
    ...
    let storageRef = '';
    let explorerUrl = '';
    let isDemo = true; // default demo for guest

    const isRealWallet = walletAddress && !walletAddress.startsWith('guest:');
    if (isRealWallet) {
      // Only anchor on-chain when real wallet is connected (not guest)
      try {
        emitZeroGEvent({ type: 'storage-upload', message: 'Anchoring on Solana devnet...' });
        const res = await fetch('/api/storage', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            predictorId: walletAddress,
            matchId: match.id,
        ...
        isDemo      = !!data.demo;
        if (!isDemo) emitZeroGEvent({ type: 'storage-success', message: 'Prediction anchored on Solana', hash: storageRef });
      } catch (err) {
        isDemo = true;
            ...
            >
              ...
              <div className="space-y-3">
                <p className="font-retro text-base text-[var(--text-muted)]">
                  Sealed on Solana. No edits after kickoff.
                </p>
                {storageResult?.demo && (!walletAddress || walletAddress.startsWith('guest:')) && (
                  <p className="font-pixel text-[8px] text-[var(--neon-yellow)] text-center mt-2">
                    ⚡ CONNECT WALLET TO ANCHOR ON-CHAIN
                  </p>
                )}
                {storageResult?.demo && walletAddress && (
                ...
                {storageResult?.demo && walletAddress && (
                    <p className="font-pixel text-[7px] text-[var(--neon-orange)] tracking-widest text-center">
                      📱 PICK SAVED LOCALLY · NOT ON-CHAIN
                    </p>
                  )}
                  {storageResult?.explorerUrl && !storageResult?.demo && (() => {
                    const txSig = storageResult.explorerUrl.split('/tx/')[1]?.split('?')[0];
                    return (
                      <>
                        {txSig && (
                          <div className="w-full bg-[var(--bg-secondary)] border border-[var(--neon-green)]/20 p-2 rounded-sm mb-2">
                            <div className="font-pixel text-[6px] text-[var(--text-muted)] mb-1 tracking-widest">SOLANA TX SIGNATURE</div>
                            <div className="font-retro text-[9px] text-[var(--neon-green)] break-all leading-relaxed">{txSig}</div>
                            <button
                              onClick={() => navigator.clipboard.writeText(txSig)}
                              className="font-pixel text-[6px] text-[var(--neon-cyan)] mt-1 hover:text-white transition-colors"
                            >
                              COPY TX
                            </button>
                          </div>
