...
export async function GET() {
  ...
  try {
    ...
    if (txlineRes.status === 'fulfilled' && txlineRes.value.ok) {
      ...
      matches = (Array.isArray(fixtures) ? fixtures : [])
        .filter((f: any) => f.CompetitionId === 72)
        .map((f: any) => {
        ...
        // worldcup26 finished ALWAYS wins — most reliable for completed matches
        ...
        if (wc?.finished === 'TRUE') {
          status    = 'final';
          gameState = 'F';
        } else if (statusId && statusId !== 1 && STATUS_ID_TO_STATUS[statusId]) {
          status    = STATUS_ID_TO_STATUS[statusId];
          gameState = STATUS_ID_TO_GAMESTATE[statusId] || 'NS';
        } else if (wc?.time_elapsed === 'live') {
          status    = 'live';
          gameState = 'H1';
        } else {
          // Only mark live if start time has passed AND it's within 3 hours
          const elapsed = Date.now() - new Date(startTime).getTime();
          if (elapsed > 0 && elapsed < 3 * 60 * 60 * 1000) {
            // Don't assume live — keep as upcoming until confirmed
            status = 'upcoming';
          }
        }
